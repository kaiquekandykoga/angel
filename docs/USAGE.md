...
| `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY` | `npm run eval` only | Project keys of the Langfuse the run reports to. Both required; either missing exits `1` before any model call. The three commands never read them. |
| `LANGFUSE_BASE_URL` | optional, `npm run eval` only | The Langfuse instance those keys belong to, default `http://localhost:3000` — see [`EVAL.md`](EVAL.md#configuration). |
